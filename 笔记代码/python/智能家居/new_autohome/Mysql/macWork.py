...
def Update_mac(First_type,Second_type,Product_num,Current_state,Ip_address,Net_port):
   # return "hah1"
    try:
        ...
        if result1 != None:
            cur = conn.cursor()
            sql4="update product_belong_to set id='%s' where (first_type,second_type,product_num) in (('%s','%s','%s'))"%(result[0],First_type,Second_type,Product_num)
            cur.execute(sql4)
            conn.commit()
            cur.close()
        else:
   ...
   # return "hah1"
    try:
        ...
        if result == None:
            ...
            for i in result1:
                cur = conn.cursor()
                sql1="update product_current_state set online_value='%s' where belong_id='%s'"%(int(i[8])-1,i[8])
                cur.execute(sql1)
                conn.commit()
                cur.close()
            cur = conn.cursor()
