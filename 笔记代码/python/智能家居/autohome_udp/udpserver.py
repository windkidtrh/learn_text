...
def handle_message(data, addr, socket):
    if "first_type" in data and "second_type" in data and "device_num" in data and "current_status" in data and "request" in data :
        ...
        try:
            ...
            Num2_message=Mysql.userWork.Product_Exist_inNum2(First_type,Second_type,int(Device_nums))
            Num12_message=Mysql.userWork.Product_Exist_inNum12(First_type,Second_type,int(Device_nums))           
    ...
    if "first_type" in data and "second_type" in data and "product_num" in data and "state" in data :
        ...
        try:
            ...
            # a=state["current_status"]
            # print first_type,second_type,product_num,state,str(a)
        except:
            f=open("log.txt",'a')  
            traceback.print_exc(file=f)  
            f.flush()  
